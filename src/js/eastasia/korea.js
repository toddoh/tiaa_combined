...
export function init_render() {
    ...
    <div class="ea-korea-hero">
        <div class="ea-korea-herotext">
            ...
            <div class="hero2">
                <p>촛불 시위로 출범한 문재인정부의 100대 국정과제</p>
                <p>이행 과정과 주요 현안에 대해 추적 및 분석합니다.</p>
            </div>
        </div>
    </div>
...
const render_policy = () => {
    ...
    <div class="policylist-action-fold" banana-status="unfolded">
    ...
    <div id="policygroup">
        ${policylist.map((i) => html`
            <div class="policy-container">
                <p class="policyname"><strong class="bolder">${i.policyname}</strong></p>
                ${i.strategies.map((st) => html`
                <div class="policy-strategycontainer" banana-status="unfolded">
                    <div class="substrategyinfo">
                        <p class="substrategyname"><strong class="bolder">${st.strategyname}</strong></p>
    ...
    const policykeywords_markup = () => html`
        ${allkeywords.map((kw) => html`
        <div class="policygroup" banana-status="folded">
            <div class="policyinfo">
                <p class="policytitle"><strong>${kw.policyname}</strong></p>
            ...
            <div class="keywords">
            ${kw.keywords.map((key) => html`
                <p class="keyword"><strong class="bolder">${key}</strong><span class="highlight"> 키워드 강조 해제</span></p>
    ...
    render(policykeywords_markup(), document.querySelector('.ea-korea-policykeywords .keywords-container'));
}

const attach_events = () => {
    var substrategyinfo_el = document.querySelectorAll('.substrategyinfo');
    for (var i=0; i < substrategyinfo_el.length; i++) {
        substrategyinfo_el[i].addEventListener('click', function (e) {
            var foldstatus = getClosest(e.target.parentNode, '.policy-strategycontainer').getAttribute('banana-status');
            if (foldstatus == 'folded') {
                getClosest(e.target.parentNode, '.policy-strategycontainer').setAttribute('banana-status', 'unfolded');
            } else if (foldstatus == 'unfolded') {
                getClosest(e.target.parentNode, '.policy-strategycontainer').setAttribute('banana-status', 'folded');
            }
        });
    }

    var desc_ac_link_el = document.querySelectorAll('.descriptions .description-action-link');
    for (var i=0; i < desc_ac_link_el.length; i++) {
        desc_ac_link_el[i].addEventListener('click', function (e) {
            var linkdata = e.target.parentNode.getAttribute('banana-link');
    ...
    var pl_ac_fd_cell = document.querySelectorAll('.policylist-action-fold');
    for (var i=0; i < pl_ac_fd_cell.length; i++) {
        pl_ac_fd_cell[i].addEventListener('click', function (e) {
            var foldstatus = e.target.parentNode.getAttribute('banana-status');
            if (foldstatus == 'folded') {
                Array.prototype.forEach.call(document.querySelectorAll('.ea-korea-policylist .substrategyinfo'), function(el, index, array){
                    getClosest(el.parentNode, '.policy-strategycontainer').setAttribute('banana-status', 'unfolded');
                });

                e.target.parentNode.querySelectorAll('.bolder').innerHTML = '모든 과제/전략 한눈에 보기';
                e.target.parentNode.setAttribute('banana-status', 'unfolded');
            } else if (foldstatus == 'unfolded') {
                Array.prototype.forEach.call(document.querySelectorAll('.ea-korea-policylist .substrategyinfo'), function(el, index, array){
                    getClosest(el.parentNode, '.policy-strategycontainer').setAttribute('banana-status', 'folded');
                });

                e.target.parentNode.querySelectorAll('.bolder').innerHTML = '모든 과제/전략 펼쳐서 보기';
                e.target.parentNode.setAttribute('banana-status', 'folded');
            }
        });
    }

    var pl_keyw_fd = document.querySelectorAll('.ea-korea-policykeywords .policyinfo');
    for (var i=0; i < pl_keyw_fd.length; i++) {
        pl_keyw_fd[i].addEventListener('click', function (e) {
            var foldstatus = getClosest(e.target.parentNode, '.policygroup').getAttribute('banana-status');
            if (foldstatus == 'folded') {
                getClosest(e.target.parentNode, '.policygroup').setAttribute('banana-status', 'unfolded');
            } else if (foldstatus == 'unfolded') {
                getClosest(e.target.parentNode, '.policygroup').setAttribute('banana-status', 'folded');
            }
        });
    }

    var pl_keyw = document.querySelectorAll('.ea-korea-policykeywords .keyword');
    for (var i=0; i < pl_keyw.length; i++) {
        pl_keyw[i].addEventListener('click', function (e) {
            ...
            if (e.target.parentNode.getAttribute('banana-status') == 'highlighted') {
                ...
                Array.prototype.forEach.call(document.querySelectorAll('.ea-korea-policylist .task'), function(el, index, array){
                    if (el.getAttribute('banana-taskdata').indexOf(key) != -1) {
